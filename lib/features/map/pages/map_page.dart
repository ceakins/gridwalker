...
class _MapPageState extends State<MapPage> {
  ...
  void _onStyleLoaded() async {
    ...
    if (mounted) {
      ...
      // Apply persisted 3D tilt if enabled
      if (state.is3D) {
        mapController?.animateCamera(CameraUpdate.tiltTo(60.0));
      }
    }
  }
  ...
  void _clearSelectionPreview() async {
    if (_isSelectionSourceAdded) {
      try { await mapController?.setGeoJsonSource("selection-source", {"type": "FeatureCollection", "features": []}); } catch (e) { _isSelectionSourceAdded = false; }
    }
  }
          ...
          return Column(
            ...
            children: [
              FloatingActionButton(
                ...
                mini: true,
                backgroundColor: state.isSatellite ? Colors.green : Colors.white,
                onPressed: () => context.read<AppBloc>().add(ToggleSatellite()),
                child: Icon(Icons.map, color: state.isSatellite ? Colors.white : Colors.green),
              ),
              const SizedBox(height: 8),
              FloatingActionButton(
                heroTag: 'toggle_3d',
                mini: true,
                backgroundColor: state.is3D ? Colors.blue : Colors.white,
                onPressed: () {
                  context.read<AppBloc>().add(Toggle3D());
                  mapController?.animateCamera(CameraUpdate.tiltTo(state.is3D ? 0.0 : 60.0));
                },
                child: Icon(Icons.terrain, color: state.is3D ? Colors.white : Colors.blue),
              ),
              const SizedBox(height: 8),
              FloatingActionButton(
                heroTag: 'add_zone',
                ...
                onPressed: () {
                  setState(() { _isDrawing = !_isDrawing; _dragStart = null; _dragEnd = null; });
                  if (_isDrawing) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Long-press and drag on map'))); }
                },
